                ...
                <div class="page-title">
                    <div class="row">
                        <div class="col-12 col-md-6 order-md-1 order-last">
                            <h3>Workers list Table</h3>
                            <p class="text-subtitle text-muted">A listed out of workers from company .</p>
                        </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table class="table table-striped" id="table1">
                                <thead>
                                    <tr>
                                        <th>Workers profile</th>
                                        <th>Workers username</th>
                                        <th>Workers phonenumber</th>
                                        <th>Workers age</th>
                                        <th>Workers kg</th>
                                        <th>Workers colors</th>
                        ...
                        <tr>
                            <td>
                                @if ($item->workerphoto)
                                    <img src="{{ asset(asset('storage/images/worker_profile/'.$item->workerphoto)) }}" alt="Worker Image" style="max-width: 100px; max-height: 100px;">
                                @else
                                    No image available
                                @endif
                            </td>
                            <td>{{ substr($item->workername, 0, 20) }}{{ strlen($item->workername) > 20 ? '...' : '' }}</td>
                            <td>{{ substr($item->workerphone, 0, 20) }}{{ strlen($item->workerphone) > 20 ? '...' : '' }}</td>
                            <td>{{ substr($item->workerage, 0, 20) }}{{ strlen($item->workerage) > 20 ? '...' : '' }}</td>
                            <td>{{ substr($item->workerkg, 0, 20) }}{{ strlen($item->workerkg) > 20 ? '...' : '' }}</td>
                            <td>{{ substr($item->workercolor, 0, 20) }}{{ strlen($item->workercolor) > 20 ? '...' : '' }}</td>
                            
                            <td>{{ $item->created_at }}</td>
                            <td>{{ $item->updated_at }}</td>                          
                            <td>
                                @if ($item->workerstatus === "active" || $item->workerstatus === null)
                                <div style="display: flex; align-items: center;">
                                    <form action="{{route('deactive_w',$item->id)}}" method="POST">
                                        @csrf
                              ...
                              @else
                               <div style="display: flex; align-items: center;">
                                <form action="{{route('active_w',$item->id)}}" method="POST">
                                    @csrf
                                    @method('POST') <!-- Use POST method -->
                           ...
                           <td>
                            <div style="display: flex; align-items: center;">
                                <a href="{{ route('edit_w', $item->id) }}" class="btn btn-success rounded-pill m-2">Edit</a>
                            </div>
                           </td>
                        </tr>
                                 @endforeach 
